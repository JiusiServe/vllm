...
@pytest.mark.parametrize("use_ec_connector, ec_role", [(False, None),
                                                       (True, "ec_consumer")])
...
def create_requests_with_priority(
    ...
    for i in range(num_requests):
        ...
        prompt_token_ids = ([starting_idx] *
                            num_tokens if same_prompt else [i + starting_idx] *
                            num_tokens)
        request = Request(
...
def _assert_right_encoder_cache_allocated(
    ...
    expected_total_allocated: Optional[int] = None,
):
...
def _assert_right_ec_connector_metadata(
    ...
    for i, mm_feature in enumerate(mm_features_list):
        ...
        assert metadata_dict[
            identifier].num_token == mm_feature.mm_position.length
...
def _assert_right_encoder_inputs(
    ...
    requests: Optional[list[Request]] = None,
    expected_encoder_inputs: Optional[list[list[int]]] = None,
    expected_total_reqs: Optional[int] = None,
):
...
def test_ec_connector_text_only_request(use_kv_connector):
    ...
    _assert_right_encoder_cache_allocated(scheduler,
                                          expected_total_allocated=0)
...
def test_ec_connector_cache_hit_external_load(use_kv_connector):
    ...
    _assert_right_ec_connector_metadata(output,
                                        mm_features_list=request.mm_features)
...
def test_ec_connector_cache_miss_computes_locally(use_kv_connector):
    ...
    request_mm_missed = create_requests(
        ...
        mm_positions=[[PlaceholderRange(offset=0,
                                        length=100)]],  # 100 MM tokens
    )[0]
...
def test_ec_connector_with_partial_cache_hit_multi_round(use_kv_connector):
    ...
    scheduler.ec_connector.has_caches = Mock(
        return_value=[False, True, False, True])
    scheduler.ec_connector.update_state_after_alloc = Mock(
        wraps=scheduler.ec_connector.update_state_after_alloc)
    ...
    # Finish request
    scheduler.finish_requests(request1.request_id,
...
@pytest.mark.parametrize("cache_exist",
                         ["local", "connector_only", "no_where"])
@pytest.mark.parametrize("use_kv_connector", [False, True])
def test_ec_connector_schedule_multiple_requests(cache_exist,
                                                 use_kv_connector):
    scheduler = create_scheduler(
        model="llava-hf/llava-1.5-7b-hf",
    ...
    if cache_exist == "connector_only":
        ...
        mm_features_list = [
            feature for req in requests for feature in req.mm_features
        ]

        # Check metadata should contain mm data for all 10 requests
        _assert_right_ec_connector_metadata(output,
                                            mm_features_list=mm_features_list)
    else:
...
def test_priority_scheduling_ec_connector_preemption_and_resumption(
    ...
    scheduler = create_scheduler_with_priority(
        ...
        use_kv_connector=use_kv_connector,
        num_blocks=
        15,  # can hold 244 tokens with 14 blocks (first block is null)
        block_size=16,  # standard block size
        use_ec_connector=True,
        ec_role="ec_consumer",
        disable_hybrid_kv_cache_manager=use_kv_connector,
